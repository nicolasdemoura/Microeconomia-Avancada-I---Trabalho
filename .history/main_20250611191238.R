...
od_data <- raw_od_data %>%
    dplyr::select(
        ...
        DIA_SEM, TOT_VIAG, ZONA_O, ZONA_D, MOTIVO_O, MOTIVO_D, DURACAO
    ) %>%
    mutate(
    ...
    ) %>% dplyr::select(-c(SEXO, DIA_SEM, DATA, ZONA_O, ZONA_D))
...
# Merge the RMSP data with the OD data on the station ID 
od_data <- od_data %>% group_by(cat_zone_o) %>%
  mutate(
    closest_station = first(RMSP$closest_station[RMSP$Zona == cat_zone_o]))

# Merge the CEMADEN data with the OD data on the date and closest station
od_data <- od_data %>%
  left_join(daily_cemaden_data, by = c("date_ref" = "day", "closest_station" = "codEstacao")) %>%
  rename(total_rain = total_rain) %>%
  mutate(total_rain = ifelse(is.na(total_rain), 0, total_rain))
