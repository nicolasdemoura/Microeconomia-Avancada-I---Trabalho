...
# Convert cemaden_vect_proj to sf for Voronoi
cemaden_sf <- st_as_sf(cemaden_vect_proj)

# Get the bounding box of RMSP for cropping
bbox <- st_bbox(st_as_sf(RMSP))

# Create Voronoi polygons
voronoi <- st_voronoi(st_union(cemaden_sf), envelope = st_as_sfc(bbox))
voronoi_sf <- st_collection_extract(st_cast(voronoi), "POLYGON")

# Assign each polygon the station ID (row order matches cemaden_sf)
voronoi_sf <- st_sf(geometry = voronoi_sf)
voronoi_sf$station_id <- cemaden_sf$station_id  # Replace with your station ID column name

# Clip Voronoi polygons to RMSP boundary
RMSP_sf <- st_as_sf(RMSP)
voronoi_clipped <- st_intersection(voronoi_sf, RMSP_sf)

# Plot the result
png("figures/voronoi_cemaden.png", width = 800, height = 600)
plot(st_geometry(RMSP_sf), col = "white", border = "black")
plot(voronoi_clipped["station_id"], add = TRUE, border = NA, alpha = 0.5)
plot(st_geometry(cemaden_sf), add = TRUE, col = "red", pch = 20, cex = 0.7)
dev.off()
