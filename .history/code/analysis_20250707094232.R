...
# Create and export descriptive statistics
descriptive_table <- export_descriptive_stats(merged_data, "output/descriptive_stats.tex")

# Print summary of results
print("Analysis completed successfully!")
print(paste("Panel 1 observations:", nobs(analysis_results$panel1$model1)))
print(paste("Panel 2 observations:", nobs(analysis_results$panel2$model1)))
...
print("Panel A: Effect on Work Probability")
print("="*50)
for(i in 1:5) {
    model_name <- paste("Model", i)
    model <- analysis_results$panel1[[i]]
    coef_summary <- summary(model)$coefficients
    
    if("dm_rain" %in% rownames(coef_summary)) {
        dm_rain_coef <- round(coef_summary["dm_rain", "Estimate"], 4)
        dm_rain_pval <- round(coef_summary["dm_rain", "Pr(>|t|)"], 4)
        print(paste(model_name, "- Rain Dummy:", dm_rain_coef, "(p-value:", dm_rain_pval, ")"))
    }
    
    if("total_rain" %in% rownames(coef_summary)) {
        total_rain_coef <- round(coef_summary["total_rain", "Estimate"], 6)
        total_rain_pval <- round(coef_summary["total_rain", "Pr(>|t|)"], 4)
        print(paste(model_name, "- Total Rain:", total_rain_coef, "(p-value:", total_rain_pval, ")"));
    }
}

# Display Panel 2 results (Work Duration)
print("Panel B: Effect on Work Duration")
print("="*50)
for(i in 1:5) {
    model_name <- paste("Model", i)
    model <- analysis_results$panel2[[i]]
    coef_summary <- summary(model)$coefficients
