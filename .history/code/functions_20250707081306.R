...
create_stations_map <- function(RMSP, colors, stations) {
...
create_closest_stations_map <- function(RMSP, stations_coords, centroids_coords) {
...
create_voronoi_map <- function(RMSP, voronoi_polygons, stations, centroids_coords) {
    n_colors <- length(levels(RMSP$closest_station))
    colors_closest <- colorRampPalette(brewer.pal(12, "Set3"))(n_colors)
...
create_rainfall_histogram <- function(daily_cemaden_data) {
...
create_trips_histogram <- function(od_data) {
    gg <- ggplot(od_data, aes(x = num_trips)) +
        geom_histogram(binwidth = 1, fill = "blue", color = "black") +
        labs(title = "Histogram of Number of Trips", x = "Number of Trips", y = "Frequency") +
        theme_minimal()
    ggsave("./figures/histogram_num_trips.png", plot = gg, width = 8, height = 6)
}

create_rainfall_by_work_histogram <- function(data) {
    gg <- ggplot(data, aes(x = total_rain, fill = as.factor(dm_worked))) +
        geom_histogram(binwidth = 1, position = "dodge", color = "black") +
        labs(title = "Histogram of Total Rainfall by Employment Status", x = "Total Rainfall (mm)", y = "Frequency", fill = "Worked") +
        theme_minimal()
    ggsave("./figures/histogram_total_rainfall_by_worked.png", plot = gg, width = 8, height = 6)
}
...
prepare_analysis_data <- function(od_data) {
    data <- od_data %>%
        mutate(
...
run_trip_analysis <- function(data) {
    reg_model <- lm(num_trips ~ total_rain, data = data)
    reg_model_dummy <- lm(num_trips ~ dm_rain, data = data)
    reg_model_fe <- feols(num_trips ~ dm_rain | cat_zone, data = data)
    
    return(list(
        continuous = reg_model,
        dummy = reg_model_dummy,
        fixed_effects = reg_model_fe
    ))
}

run_work_analysis <- function(data) {
    reg_model_worked <- feols(dm_worked ~ dm_rain + total_rain | cat_zone, data = data[data$dm_employed == 1, ])
    return(reg_model_worked)
}