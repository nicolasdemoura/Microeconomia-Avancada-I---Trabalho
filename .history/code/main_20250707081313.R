...
analysis_data <- prepare_analysis_data(merged_data)

# Create additional visualizations
create_rainfall_by_work_histogram(analysis_data)

# Run trip analysis
trip_models <- run_trip_analysis(analysis_data)
summary(trip_models$continuous)
summary(trip_models$dummy)
summary(trip_models$fixed_effects)

# Run work analysis
work_model <- run_work_analysis(analysis_data)
summary(work_model)

# Create base rainfall histogram
hist(analysis_data$total_rain, breaks = 100, main = "Histogram of Total Rainfall", xlab = "Total Rainfall (mm)", col = "lightblue")
