...
gg_rmsp_voronoi <- function(RMSP, stations, voronoi_polygons, title = "", subtitle = NULL,
    ...
    # Crop Voronoi polygons to RMSP boundary
    ...
    # Add Voronoi polygons if requested (cropped to RMSP)
    if (show_voronoi) {
        p <- p + geom_sf(data = voronoi_cropped, fill = NA, color = voronoi_color, 
                        size = 0.8, alpha = 0.8)
    }
    
    # Add stations if requested (only those within RMSP)
    if (show_stations) {
...
gg_rmsp_closest_stations <- function(RMSP, stations, title = "", subtitle = NULL,
    ...
    p <- p + 
        ...
        coord_sf(xlim = st_bbox(RMSP_sf)[c(1,3)], 
                 ylim = st_bbox(RMSP_sf)[c(2,4)], 
                 expand = FALSE) +
        scale_fill_manual(values = color_palette, guide = guide_legend(nrow = 2))
...
gg_rmsp_base <- function(RMSP, title = "", subtitle = NULL,
                        ...
                        fill_color = "#f8f9fa", border_color = "black") {
