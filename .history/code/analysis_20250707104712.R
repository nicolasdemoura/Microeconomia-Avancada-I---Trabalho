...
export_regression_table <- function(models, filename = "output/regression_results.tex") {
    ...
    panel_a_table <- modelsummary(
        ...
        escape = FALSE,
        notes = "Sample: Individuals with employment activity status (cat_actv) equal to 1 (employed) or 2 (unemployed). The dependent variable is a binary indicator equal to 1 if the individual worked on the survey day, 0 otherwise. Models: (1) Rain dummy only; (2) Total rainfall only; (3) Both rainfall variables; (4) Both rainfall variables plus individual controls; (5) Model 4 plus zone fixed effects. Individual controls include age (num_age), gender dummy (dm_male), education level (cat_educ), activity status (cat_actv), occupation (cat_ocup), economic sector (cat_setr), and employment type (cat_vinc). Zone fixed effects are municipality/zone dummies (cat_zone) to control for time-invariant spatial characteristics. Standard errors are heteroskedasticity-robust (HC1) and are shown in parentheses. Significance: * p<0.1, ** p<0.05, *** p<0.01.",
