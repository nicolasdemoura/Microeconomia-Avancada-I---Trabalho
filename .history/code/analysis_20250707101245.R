...
export_regression_table <- function(models, filename = "output/regression_results.tex") {
    ...
    # Create the LaTeX table manually for proper stacking
    latex_table <- paste(
        "\\begin{table}[htbp]",
        "\\centering",
    ...
    # Extract Panel A coefficients
    panel_a_coefs <- extract_panel_coefs(models$panel1_models)
    latex_table <- paste(latex_table, panel_a_coefs, sep = "\n")
    ...
    # Extract Panel B coefficients
    panel_b_coefs <- extract_panel_coefs(models$panel2_models)
    latex_table <- paste(latex_table, panel_b_coefs, sep = "\n")
    
    # Add observations and controls info
    latex_table <- paste(
        latex_table,
        "\\addlinespace[1em]",
        "\\midrule",
        ...
        "\\begin{tablenotes}",
        "\\footnotesize",
        "\\item \\textbf{Notes:} This table presents the effects of rainfall on labor supply outcomes.",
        "\\item Panel A shows the effect on work probability (0/1) for employed individuals (\\textit{cat\\_actv} $\\in$ \\{1,2\\}).",
        "\\item Panel B shows the effect on work duration (minutes) for employed individuals who worked.",
        ...
        "\\item Individual controls include: age (\\textit{num\\_age}), gender (\\textit{dm\\_male}), education level (\\textit{cat\\_educ}),",
        "\\item \\phantom{Individual controls incluem:} atividade (\\textit{cat\\_actv}), ocupação (\\textit{cat\\_ocup}), setor econômico (\\textit{cat\\_setr}), e tipo de emprego (\\textit{cat\\_vinc}).",
        "\\item Zone fixed effects control for unobserved time-invariant characteristics at the zone level (\\textit{cat\\_zone}).",
