...
export_regression_table <- function(models, filename = "output/regression_results.tex") {
    ...
    vcov = "HC1",
    coef_map = c("dm_rain" = "Rain Dummy (>0mm)", "total_rain" = "Total Rainfall (mm)"),
    stars = c('*' = .1, '**' = .05, '***' = .01),
    gof_omit = "AIC|BIC|Log.Lik|F|RMSE|Std.Errors|R2|Adj",
    title = "Panel A: Effect of Rainfall on Work Probability",
    escape = FALSE,
    notes = c(
        "\\textbf{Sample:} Individuals with employment activity status (cat\\_actv) equal to 1 (employed) or 2 (unemployed).",
