...
temp_cemaden_data$data_ref <- substr(temp_cemaden_data$datahora, 1, 10)
temp_cemaden_data$data_ref <- as.Date(temp_cemaden_data$data_ref, format = "%Y-%m-%d")

# Group by day and station, and calculate the sum of the valorMedida
daily_cemaden_data <- temp_cemaden_data %>%
    group_by(data_ref, station_id) %>%
    summarise(
        station_id = first(station_id),
        data_ref = first(data_ref),
...
od_data <- raw_od_data %>%
    ...
    group_by(ID_PESS, date) %>% 
    summarise(
        cat_weekday = first(cat_weekday),
        num_age = first(IDADE),
        dm_male = first(dm_male)
